...
O bloco de tamanho mostra também o **render no forno** (ver `04-desenharForno.md`) e um **slider de ajuste**: por padrão ele escala a peça **proporcionalmente** (preserva a razão entre as medidas); ao tocar em um campo de medida, o slider passa a controlar **só aquela dimensão**.
...
- **Render integrado**: o bloco de tamanho mostra o seletor de forno e o render-duplo (ver `04-desenharForno.md`).
...
- `cubagemCm3` (number ≥ 0).
- `estimarCabem(...)` → `{ total, porNivel, niveis, mode }` (ver `Specs/queima/01-estimarCabem.md`).
