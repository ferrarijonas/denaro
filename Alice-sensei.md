...
| R15 | Decisão do usuário: GitHub Pages é a hospedagem única | **Fim do servidor local (`node server.js`).** Todo o mock passa a rodar 100% estático em `https://ferrarijonas.github.io/denario/` com `localStorage` como fonte de verdade (LS_KEY `alice-mock-custos-v3`); exportar/importar backup é a portabilidade. Workflow `pages.yml` público. Nada de paths absolutos; `BACKEND_URL` fica vazia. | T34, T35 |
|    |                        |             |                  |
