...
- **Ordem de arquivos:** prefixo numérico `NN-` em specs de um módulo. A ordem de leitura vive em `Specs/.ordem.json`. Arquivo novo → adicione à lista na posição certa e rode `node ZenSpecKit/reorder.js --fix`.
- **Verificação:** antes de terminar qualquer mudança de spec, rode `node ZenSpecKit/reorder.js --check` e `node tools/snapshot.js --check`.
...
- Não criar pasta/arquivo de spec novo sem registrar a posição no `Specs/.ordem.json`.